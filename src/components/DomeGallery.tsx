...
type ImageItem = string | { src: string; alt?: string; description?: string };
...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  const usedImages: Array<(typeof normalizedImages)[number]> = [];
  for (let i = 0; i < totalSlots; i++) {
    let pick = normalizedImages[Math.floor(Math.random() * normalizedImages.length)];
    if (normalizedImages.length > 1 && i > 0) {
      const previous = usedImages[i - 1];
      if (pick.src === previous.src) {
        for (let attempt = 0; attempt < 6; attempt++) {
          const candidate = normalizedImages[Math.floor(Math.random() * normalizedImages.length)];
          if (candidate.src !== previous.src) {
            pick = candidate;
            break;
          }
        }
...
export default function DomeGallery({
      ...
      onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
        ...
        if (last) {
          ...
          if (startPosRef.current) {
            ...
            const tapThresholdPx = pointerTypeRef.current === 'touch' ? 10 : 6;
            if (dist2 <= tapThresholdPx * tapThresholdPx) {
              isTap = true;
            }
          }
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    if (rawAlt || rawDescription) {
      const caption = document.createElement('div');
      caption.style.cssText =
        'position:absolute;left:0;right:0;bottom:0;padding:12px 14px;background:linear-gradient(180deg, rgba(0,0,0,0) 0%, rgba(0,0,0,0.78) 100%);color:#fff;pointer-events:none;';

      if (rawAlt) {
        const title = document.createElement('p');
        title.textContent = rawAlt;
        title.style.cssText = 'margin:0;font-size:15px;font-weight:600;line-height:1.2;';
        caption.appendChild(title);
      }

      if (rawDescription) {
        const description = document.createElement('p');
        description.textContent = rawDescription;
        description.style.cssText = 'margin:4px 0 0 0;font-size:12px;line-height:1.35;opacity:0.92;';
  ...
  return (
    <>
      ...
      <div
        ...
        style={
          {
            ['--segments-x' as const]: segments,
            ['--segments-y' as const]: segments,
            ['--overlay-blur-color' as const]: overlayBlurColor,
            ['--tile-radius' as const]: imageBorderRadius,
            ['--enlarge-radius' as const]: openedImageBorderRadius,
            ['--image-filter' as const]: grayscale ? 'grayscale(1)' : 'none'
          } as CSSProperties
        }
      >
        <main
            ...
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                <div
                  ...
                  style={
                    {
                      ['--offset-x' as const]: it.x,
                      ['--offset-y' as const]: it.y,
                      ['--item-size-x' as const]: it.sizeX,
                      ['--item-size-y' as const]: it.sizeY,
                      top: '-999px',
                      bottom: '-999px',
                      left: '-999px',
                      right: '-999px'
